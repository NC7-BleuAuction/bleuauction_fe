...
function MarketDetailPage() {
  ...
  const storeInfo = {
    ...
    name: '대원수산',
    location:
      '서울특별시 동작구 노들로 674 노량진수산시장 , 2층 활어 2-15호 강변상회',
  ...
  useEffect(() => {
    if (store && store.storeNo) {
      ...
      sendAxiosRequest(
        ...
        (error) => {
          console.error(
            'An error occurred while fetching the store details:',
            error
          );
        }
      );
  ...
  return (
    <div>
      ...
      <div style={tabContainerStyle}>
        ...
        {activeTab === 'menu' && (
          <Button onClick={handleOrderClick} buttonText="주문하기" />
        )}
        <OrderModal
          store={store}
          menus={menuData}
