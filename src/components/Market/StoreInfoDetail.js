...
import {getCurrentDay, isStoreOpen} from '../utility/common';
    ...
    return (
        <div style={outerContainer}>
            <div style={storeInfoContainer}>
                ...
                <ul style={infoList}>
                    <li style={listItem}><FaRegBuilding style={{marginRight: '10px'}}/>시장
                        이름: {storeDetail.marketName}</li>
                    <li style={listItem}><FaMapMarkerAlt style={{marginRight: '10px'}}/>기본
                        주소: {storeDetail.storeAddr}</li>
                    <li style={listItem}><FaMapMarkerAlt style={{marginRight: '10px'}}/>상세
                        주소: {storeDetail.storeDetailAddr}</li>
                    <div>
                        <li style={listItem}>
                            <FaClock style={{marginRight: '10px'}}/>
                            평일 운영 시간: {storeDetail.weekdayStartTime} ~ {storeDetail.weekdayEndTime}
                            {currentDay >= 1 && currentDay <= 5 && isStoreOpen(storeDetail.weekdayStartTime, storeDetail.weekdayEndTime, currentDay)}
                        </li>
                        <li style={listItem}>
                            <FaClock style={{marginRight: '10px'}}/>
                            주말 운영 시간: {storeDetail.weekendStartTime} ~ {storeDetail.weekendEndTime}
                            {currentDay === 0 || currentDay === 6 && isStoreOpen(storeDetail.weekendStartTime, storeDetail.weekendEndTime, currentDay)}
                        </li>
                    </div>
