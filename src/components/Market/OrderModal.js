...
const OrderModal = ({store, menus, isOpen, onClose, setMenuData}) => {
    ...
    const handleOrder = (event) => {
        ...
        // formData.append('orderPrice', order.orderPrice);
        // formData.append('orderRequest', order.orderRequest);
        // formData.append('recipientPhone', order.recipientPhone);
        // formData.append('recipientName', order.recipientName);
        // formData.append('recipientZipcode', order.recipientZipcode);
        // formData.append('recipientAddr', order.recipientAddr);
        // formData.append('recipientDetailAddr', order.recipientDetailAddr);
        ...
        console.log('order.orderorder.orderorder.order', order);
        ...
        sendAxiosRequest('/api/order/new', 'POST', order, response => {
            ...
            if(isNotNullOrNonEmpty(response.data)) {
                sendAxiosRequest('/api/order/orderMenus', 'POST', orderMenus, response2 => {
                    console.log(response2.data);


                }, error2 => {
                    console.log('두 번째 요청 에러:', error2);
                }, null, accessToken);
            }

        }, error => {
            console.log('첫 번째 요청 에러:', error);
        }, null, accessToken);

        // sendAxiosRequest('/api/order/new', 'POST', orderMenus, response => {
        //
        //     console.log(response.data);
        //
        ...
        // 주문 생성
        // sendAxiosRequest('/api/order/new', 'POST', formData, response => {
        ...
        // sendAxiosRequest('/api/ordermenu/new', 'POST', orderMenus, response => {
        //     if (response.data && response.data.length > 0) {
