...
const OrderModal = ({ store, menus, isOpen, onClose, setMenuData }) => {
  ...
  const handleOrder = (event) => {
    ...
    axios
      .post('/api/order/new', data, {
        headers: {
          'Content-Type': 'multipart/form-data', // JSON 형식으로 요청
        },
      })
      .then(
        (response) => console.log('주문 생성에 성공했습니다:', response.data),
        console.log('order=> : ', order)
      )
    ...
    <div className="orderModalBackground">
      <div className="orderModalContainer">
        ...
        <div className="sectionButtons">
          ...
          <button onClick={() => switchSection('deliveryMethod')}>
            수령 방식
          </button>
        </div>
        ...
        {activeSection === 'deliveryMethod' && (
          <div>
            ...
            <div className="delivery">
              ...
              <Payment orderData={order} onClick={handleOrder}>
                결제하기
              </Payment>
            </div>
          </div>
