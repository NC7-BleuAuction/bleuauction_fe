...
function StroeItemAdd() {
  ...
  const selectPlace = origin === 'D' ? DPlace : origin === 'I' ? IPlace : null;
    ...
    <>
      ...
      <div className="freezer_box">
        {Object.keys(fishs).map((name) => (
          ...
          >
            <img
              src={`/images/fish${fishs[name]}.jpg`}
              alt={name}
              className="fish_image"
            />
            <p>{name}</p>
          </div>
        ))}
      ...
      <div className="fish_size">
        ...
        <div>
          <input
            ...
            required
          />
          <p className=" ba-margin-top20 ba-margin-bottom150">
            사이즈 측정 :{' '}
            <b className="ba-important-text">
              {itemSize === 'L'
                ? '대 (3kg 이상)'
                : itemSize === 'M'
                ? '중 (1kg ~ 2kg)'
      ...
      <div className="fish_origin">
        ...
        <div className="origin_buttons">
          <button
            className={wild === 'W' ? 'selected' : ''}
            onClick={() => setWild('W')}
          >
            자연산
          </button>
          <button
            className={wild === 'F' ? 'selected' : ''}
            onClick={() => setWild('F')}
          >
            양식
          </button>
