...
function StoreEditPage() {
  ...
  useEffect(() => {
    if (loginUser) {
      sendAxiosRequest(
        `/api/store/detailByMember?member=${loginUser?.memberNo}`,
        'GET',
        null,
        (res) => {
          setStore(res.data);
