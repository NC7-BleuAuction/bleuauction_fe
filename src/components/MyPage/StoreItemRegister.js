...
function StoreItemRegister() {
  ...
  useEffect(() => {
    const itemCodeSelect = document.getElementById('itemCode');
    const originStatusSelects = document.querySelectorAll(
      '[name=originStatus]'
    );
      ...
      <form id="sidpForm" className="ba-form-container">
        <table>
          ...
          <tbody>
            <tr>
              <th>
                <label htmlFor="daliyPriceDate">기준날짜</label>
              </th>
              <td>
            ...
            <tr>
              <th>
                <label htmlFor="item_code">품목구분</label>
              </th>
              <td>
            ...
            <tr>
              <th>
                <label htmlFor="originPlaceStatus">지역명</label>
              </th>
              <td>
        ...
        <button
          type="button"
          className="ba-btn ba-margin-tb50"
          onClick={() => insertSidp(tokenMember)}
        >
          품목시세 등록
        </button>
      </form>
